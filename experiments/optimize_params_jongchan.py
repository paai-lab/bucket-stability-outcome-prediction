...
def create_and_evaluate_model(args):
    ...
    elif "cluster" in method_name:
        for k, v in args.items():
            fout_all.write("%s;%s;%s;%s;%s;%s;%s;%s\n" % (trial_nr, dataset_name, cls_method, method_name, n_clusters, k, v, score / n_splits))   
        fout_all.write("%s;%s;%s;%s;%s;%s;%s;%s\n" % (trial_nr, dataset_name, cls_method, method_name, n_clusters, "processing_time", time.time() - start, 0))
    else:
        for k, v in args.items():
...
cls_encoding = argv[5]
cls_method = argv[6]
max_prefix = int(argv[7])
if bucket_method == "cluster":
    n_clusters = int(argv[8])
...
for dataset_name in datasets:
    ...
    if cls_method == "rf":
        ...
        space = {'learning_rate': hp.uniform("learning_rate", 0, 1),
                 'min_samples_split': scope.int(hp.quniform('min_samples_split', 4, 30, 1)),
                 'max_depth': scope.int(hp.quniform('max_depth', 4, 30, 1))}
    elif cls_method == "logit":
    ...
    if bucket_method == "cluster":
        space['n_clusters'] = n_clusters

    # optimize parameters
    trial_nr = 1
    trials = Trials()
    if "cluster" in method_name:
        fout_all = open(os.path.join(params_dir, "param_optim_all_trials_%s_%s_%s_%s.csv" % (cls_method, dataset_name, method_name, n_clusters)), "w")
    else:
    ...
    if "prefix" in method_name:
        ...
        fout_all.write("%s;%s;%s;%s;%s;%s;%s;%s\n" % ("iter", "dataset", "cls", "method", "n_clusters", "param", "value", "score"))
    else:
    ...
    if "cluster" in method_name:
        outfile = os.path.join(params_dir, "optimal_params_%s_%s_%s_%s.pickle" % (cls_method, dataset_name, method_name, n_clusters))
    else:
